/** Five-pointed star, gold when filled, muted outline when not — supports
 *  per-review ratings rather than assuming every review is 5 stars. */
export function Star({ filled = true, className = "h-4 w-4" }: { filled?: boolean; className?: string }) {
  const path =
  ...
  return (
    <svg viewBox="0 0 20 20" className={className} aria-hidden>
      <path d={path} fill={filled ? "#FBBC05" : "none"} stroke={filled ? "none" : "rgba(255,255,255,0.25)"} />
    </svg>
  );
}
